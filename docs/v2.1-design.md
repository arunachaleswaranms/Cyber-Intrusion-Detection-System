...
Status: **Design complete; Phases 1–2 implemented**
...
configs/
├── v2.1-workbench-v1.json         # Rejected TreeExplainer proposal
└── v2.1-workbench-v2.json         # Active resource and explanation policy
tests/
...
SHAP's model-agnostic `PermutationExplainer` is approved by accepted
[ADR 0003](decisions/0003-v2.1-explanation-fallback.md) after the original
TreeExplainer route failed selected-artifact additivity. The pinned Python 3.12
technical spike demonstrated all of the following for both selected Histogram
Gradient Boosting models before UI implementation:
...
If the fallback gate fails, v2.1 must limit explainability to global permutation
importance calculated on development data. Permutation importance is global
model reliance, not a per-record explanation.
...
Implementation status: **complete**. The contracts, evidence loader, model-pack
preflight, representative tests, and
[selected-artifact SHAP gate](shap-compatibility-gate.md) passed. ADR 0003 is
accepted, so Phase 2 may use the bounded explanation contract where applicable.
...
Implementation status: **complete in evidence mode**. See
[`dashboard.md`](dashboard.md). The Overview, Binary detection, Attack families,
Explainability-status and Provenance pages run from a clean clone without a
dataset or model. Page code lives in `src/cids/dashboard/` with
`dashboard/app.py` as the entry point. The Explainability page shows gate
status and limitations only, because the committed gate report contains no
attributions. `AppTest` smoke tests were added early; container checks remain
Phase 5.
...
- [x] Evidence mode starts from a clean clone with no dataset or model.
- [x] Every displayed frozen number is derived from the committed v2.0 JSON and
...
- [SHAP TreeExplainer](https://shap.readthedocs.io/en/stable/generated/shap.TreeExplainer.html): documents the rejected tree-model route and its additivity check.
- [SHAP PermutationExplainer](https://shap.readthedocs.io/en/stable/generated/shap.PermutationExplainer.html): documents the bounded model-agnostic fallback proposed by ADR 0003.
