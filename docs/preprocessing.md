...
`unsw-nb15-split-v1` and fits only `PreparedSplits.train`. It verifies the
training-ID SHA-256 from the split report before fitting. Validation and official
test records are transformed only after the categorical vocabulary is fixed.
...
- Categorical vocabulary sizes, training row count, split policy, schema version,
  task, and training-ID digest are stored as provenance metadata.
